...
# There should now be a file named `elm.gz` in the `/tmp` directory.
#
...
# There should now be a file named `elm` in the `/tmp` directory.
#
...
# The `elm` file is now executable. That means running `/tmp/elm --help`
